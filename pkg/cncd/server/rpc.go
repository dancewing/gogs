...
import (
	...
	"github.com/gogits/gogs/pkg/cncd/remote"
)
...
var Config = struct {
	Services struct {
		...
		Queue      queue.Queue
		Logs       logging.Log
		Senders    models.SenderService
		Secrets    models.SecretService
		Registries models.RegistryService
		Environ    models.EnvironService
	}
	Storage struct {
		// Users  models.UserStore
		// Repos  models.RepoStore
		// Builds models.BuildStore
		// Logs   models.LogStore
		Config models.ConfigStore
		Files  models.FileStore
		Procs  models.ProcStore
		// Registries models.RegistryStore
...
func (s *RPC) checkCancelled(pipeline *rpc.Pipeline) (bool, error) {
	...
	if proc.State == models.StatusKilled {
		return true, nil
	}
	return false, err
}
