...
func renderDirectory(c *context.APIContext, data map[string]interface{}, treeLink string) {
	...
	data["Files"], err = entries.GetCommitsInfoWithCustomConcurrency(c.Repo.Commit, c.Repo.TreePath, setting.Repository.CommitsFetchConcurrency)
	if err != nil {
	...
	if readmeFile != nil {
		data["RawFileLink"] = ""
		data["ReadmeInList"] = true
		data["ReadmeExist"] = true
		...
		data["IsTextFile"] = isTextFile
		data["FileName"] = readmeFile.Name()
		if isTextFile {
			...
			case markup.IPYTHON_NOTEBOOK:
				data["IsIPythonNotebook"] = true
				data["RawFileLink"] = c.Repo.RepoLink + "/raw/" + path.Join(c.Repo.BranchName, c.Repo.TreePath, readmeFile.Name())
			default:
				buf = bytes.Replace(buf, []byte("\n"), []byte(`<br>`), -1)
			}
			data["FileContent"] = string(buf)
		}
	}
	...
	data["LatestCommitUser"] = models.ValidateCommitWithEmail(latestCommit)

	if c.Repo.CanEnableEditor() {
		data["CanAddFile"] = true
		data["CanUploadFile"] = setting.Repository.Upload.Enabled
	}
}

func renderFile(c *context.APIContext, data map[string]interface{},  entry *git.TreeEntry, treeLink, rawLink string) {
	data["IsViewFile"] = true
	...
	data["HighlightClass"] = highlight.FileNameToHighlightClass(blob.Name())
	data["RawFileLink"] = rawLink + "/" + c.Repo.TreePath
	...
	if !isTextFile {
		data["EditFileTooltip"] = c.Tr("repo.editor.cannot_edit_non_text_files")
	}
	...
	case isTextFile:
		if blob.Size() >= setting.UI.MaxDisplayFileSize {
			data["IsFileTooLarge"] = true
			break
		}

		data["ReadmeExist"] = markup.IsReadmeFile(blob.Name())
		...
		case markup.MARKDOWN:
			data["IsMarkdown"] = true
			data["FileContent"] = string(markup.Markdown(buf, path.Dir(treeLink), c.Repo.Repository.ComposeMetas()))
		case markup.ORG_MODE:
			data["IsMarkdown"] = true
			data["FileContent"] = string(markup.OrgMode(buf, path.Dir(treeLink), c.Repo.Repository.ComposeMetas()))
		case markup.IPYTHON_NOTEBOOK:
			data["IsIPythonNotebook"] = true
		default:
			...
			data["FileContent"] = gotemplate.HTML(output.String())

			output.Reset()
			for i := 0; i < len(lines); i++ {
				output.WriteString(fmt.Sprintf(`<span id="L%d">%d</span>`, i+1, i+1))
			}
			data["LineNums"] = gotemplate.HTML(output.String())
		}

		if canEnableEditor {
			data["CanEditFile"] = true
			data["EditFileTooltip"] = c.Tr("repo.editor.edit_this_file")
		} else if !c.Repo.IsViewBranch {
			data["EditFileTooltip"] = c.Tr("repo.editor.must_be_on_a_branch")
		} else if !c.Repo.IsWriter() {
			data["EditFileTooltip"] = c.Tr("repo.editor.fork_before_edit")
		}
	...
	if canEnableEditor {
		data["CanDeleteFile"] = true
		data["DeleteFileTooltip"] = c.Tr("repo.editor.delete_this_file")
	} else if !c.Repo.IsViewBranch {
		data["DeleteFileTooltip"] = c.Tr("repo.editor.must_be_on_a_branch")
	} else if !c.Repo.IsWriter() {
		data["DeleteFileTooltip"] = c.Tr("repo.editor.must_have_write_access")
	}
}

func setEditorconfigIfExists(c *context.APIContext) {
...
func Home(c *context.APIContext) {

	data := make(map[string]interface{})

	data["PageIsViewFiles"] = true
	...
	if c.Repo.BranchName != c.Repo.Repository.DefaultBranch {
		data["Title"] = title + " @ " + c.Repo.BranchName
	}
	...
	if len(c.Repo.TreePath) > 0 {
		...
		data["CommitsCount"] = c.Repo.CommitsCount
	}
	data["PageIsRepoHome"] = isRootDir
	...
	if entry.IsDir() {
		renderDirectory(c, data, treeLink)
	} else {
		renderFile(c, data, entry, treeLink, rawLink)
	}
	if c.Written() {
		return
	}
	...
	if len(c.Repo.TreePath) > 0 {
		...
		data["HasParentPath"] = true
		if len(paths)-2 >= 0 {
			data["ParentPath"] = "/" + paths[len(paths)-2]
		}
	}
