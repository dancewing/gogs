...
func Search(c *context.APIContext) {
	...
	result := make([]*api.Repository, len(repos))
	for i := range repos {
		result[i] = api.ConvertRepositoryToAPI(repos[i], &api.Permission{true, true, true})
	}
...
func listUserRepositories(c *context.APIContext, username string) {
	...
	if c.User.ID != user.ID {
		...
		for i := range ownRepos {
			repos[i] = api.ConvertRepositoryToAPI(ownRepos[i], &api.Permission{true, true, true})
		}
		c.JSON(200, &repos)
		return
	}
	...
	for repo, access := range accessibleRepos {
		repos[i] = api.ConvertRepositoryToAPI(repo, &api.Permission{
...
func CreateUserRepo(c *context.APIContext, owner *models.User, opt api.CreateRepoOption) {
	...
	c.JSON(201, api.ConvertRepositoryToAPI(repo, &api.Permission{true, true, true}))
}
...
func Get(c *context.APIContext) {
	...
	c.JSON(200, api.ConvertRepositoryToAPI(repo, &api.Permission{
...
func ListForks(c *context.APIContext) {
	...
	for i := range forks {
		...
		apiForks[i] = api.ConvertRepositoryToAPI(forks[i], &api.Permission{
