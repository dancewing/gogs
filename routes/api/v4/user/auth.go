...
func AutoLogin(c *context.RestContext) (bool, error) {
	if !models.HasEngine {
		return false, nil
	}
...
// return current user
func Login(c *context.RestContext) {
...
func afterLogin(c *context.RestContext, u *models.User, remember bool) {
	if remember {
...
func LoginPost(c *context.RestContext, f form.SignIn) {
	c.Data["Title"] = c.Tr("sign_in")
...
func LoginTwoFactor(c *context.RestContext) {
...
func LoginTwoFactorPost(c *context.RestContext) {
...
func LoginTwoFactorRecoveryCode(c *context.RestContext) {
	_, ok := c.Session.Get("twoFactorUserID").(int64)
	if !ok {
		c.NotFound()
		return
...
func LoginTwoFactorRecoveryCodePost(c *context.RestContext) {
	userID, ok := c.Session.Get("twoFactorUserID").(int64)
	if !ok {
		c.NotFound()
		return
...
func SignOut(c *context.RestContext) {
...
func SignUp(c *context.RestContext) {
...
func SignUpPost(c *context.RestContext, cpt *captcha.Captcha, f form.Register) {
	c.Data["Title"] = c.Tr("sign_up")
	...
	if setting.Service.RegisterEmailConfirm && u.ID > 1 {
		mailer.SendActivateAccountMail(c.Context.Context, models.NewMailerUser(u))
...
func Activate(c *context.RestContext) {
	code := c.Query("code")
	if len(code) == 0 {
		...
		if setting.Service.RegisterEmailConfirm {
			if c.Cache.IsExist("MailResendLimit_" + c.User.LowerName) {
				...
				c.Data["Hours"] = setting.Service.ActiveCodeLives / 60
				mailer.SendActivateAccountMail(c.Context.Context, models.NewMailerUser(c.User))
...
func ActivateEmail(c *context.RestContext) {
	code := c.Query("code")
...
func ForgotPasswd(c *context.RestContext) {
...
func ForgotPasswdPost(c *context.RestContext) {
	c.Data["Title"] = c.Tr("auth.forgot_password")
	...
	mailer.SendResetPasswordMail(c.Context.Context, models.NewMailerUser(u))
	if err = c.Cache.Put("MailResendLimit_"+u.LowerName, u.LowerName, 180); err != nil {
		log.Error(4, "Set cache(MailResendLimit) fail: %v", err)
	}
...
func ResetPasswd(c *context.RestContext) {
...
func ResetPasswdPost(c *context.RestContext) {
	c.Data["Title"] = c.Tr("auth.reset_password")
...
func GetAccount(c *context.RestContext) {
	if c.IsLogged {
