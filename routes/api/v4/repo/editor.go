...
func editFile(c *context.RestContext, isNewFile bool) {
	c.PageIs("Edit")
...
func EditFile(c *context.RestContext) {
	editFile(c, false)
}

func NewFile(c *context.RestContext) {
	editFile(c, true)
}

func editFilePost(c *context.RestContext, f form.EditRepoFile, isNewFile bool) {
	c.PageIs("Edit")
	c.RequireHighlightJS()
	c.RequireSimpleMDE()
	c.Data["IsNewFile"] = isNewFile
...
func EditFilePost(c *context.RestContext, f form.EditRepoFile) {
	editFilePost(c, f, false)
}

func NewFilePost(c *context.RestContext, f form.EditRepoFile) {
	editFilePost(c, f, true)
}

func DiffPreviewPost(c *context.RestContext, f form.EditPreviewDiff) {
	treePath := c.Repo.TreePath

	entry, err := c.Repo.Commit.GetTreeEntryByPath(treePath)
	if err != nil {
 		//c.Error(500, "GetTreeEntryByPath: "+err.Error()) 	//TODO
		return
	} else if entry.IsDir() {
		//c.Error(422) //TODO
		return
	...
	if err != nil {
		//c.Error(500, "GetDiffPreview: "+err.Error()) //TODO
		return
	}
...
func DeleteFile(c *context.RestContext) {
...
func DeleteFilePost(c *context.RestContext, f form.DeleteRepoFile) {
	c.Data["PageIsDelete"] = true
	c.Data["BranchLink"] = c.Repo.RepoLink + "/src/" + c.Repo.BranchName
	c.Data["TreePath"] = c.Repo.TreePath
...
func renderUploadSettings(c *context.RestContext) {
...
func UploadFile(c *context.RestContext) {
...
func UploadFilePost(c *context.RestContext, f form.UploadRepoFile) {
	c.Data["PageIsUpload"] = true
	renderUploadSettings(c)
...
func UploadFileToServer(c *context.RestContext) {
	...
	if err != nil {
		//c.Error(500, fmt.Sprintf("NewUpload: %v", err)) //TODO
		return
	}
...
func RemoveUploadFileFromServer(c *context.RestContext, f form.RemoveUploadFile) {
	if len(f.File) == 0 {
		c.Status(204)
		return
	}
	...
	if err := models.DeleteUploadByUUID(f.File); err != nil {
		//c.Error(500, fmt.Sprintf("DeleteUploadByUUID: %v", err)) //TODO
		return
	}
