...
func MustBeNotBare(c *context.RestContext) {
	if c.Repo.Repository.IsBare {
		c.Handle(404, "MustBeNotBare", nil)
	}
}

func checkContextUser(c *context.RestContext, uid int64) *models.User {
...
func Create(c *context.RestContext) {
...
func handleCreateError(c *context.RestContext, owner *models.User, err error, name, tpl string, form interface{}) {
	switch {
...
func CreatePost(c *context.RestContext, f form.CreateRepo) {
	c.Data["Title"] = c.Tr("new_repo")
...
func Migrate(c *context.RestContext) {
...
func MigratePost(c *context.RestContext, f form.MigrateRepo) {
	c.Data["Title"] = c.Tr("new_migrate")
...
func Action(c *context.RestContext) {
	var err error
...
func Download(c *context.RestContext) {
	var (
		uri         = c.Params("*")
		refName     string
		ext         string
