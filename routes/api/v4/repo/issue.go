...
func MustEnableIssues(c *context.RestContext) {
...
func MustAllowPulls(c *context.RestContext) {
...
func RetrieveLabels(c *context.RestContext) {
...
func issues(c *context.RestContext, isPullList bool) {
	if isPullList {
		MustAllowPulls(c)
		if c.Written() {
			return
...
func Board(c *context.RestContext) {
...
func Issues(c *context.RestContext) {
	c.Data["PageIsList"] = true
	issues(c, false)
}

func Pulls(c *context.RestContext) {
	c.Data["PageIsList"] = true
	issues(c, true)
}

func renderAttachmentSettings(c *context.RestContext) {
...
func RetrieveRepoMilestonesAndAssignees(c *context.RestContext, repo *models.Repository) {
	var err error
...
func RetrieveRepoMetas(c *context.RestContext, repo *models.Repository) []*models.Label {
	if !c.Repo.IsWriter() {
		return nil
	}
...
func getFileContentFromDefaultBranch(c *context.RestContext, filename string) (string, bool) {
	var r io.Reader
	var bytes []byte
...
func setTemplateIfExists(c *context.RestContext, ctxDataKey string, possibleFiles []string) {
...
func NewIssue(c *context.RestContext) {
...
func ValidateRepoMetas(c *context.RestContext, f form.NewIssue) ([]int64, int64, int64) {
	var (
		repo = c.Repo.Repository
		err  error
	)
...
func NewIssuePost(c *context.RestContext, f form.NewIssue) {
	c.Data["Title"] = c.Tr("repo.issues.new")
	c.Data["PageIsIssueList"] = true
	c.Data["RequireHighlightJS"] = true
	c.Data["RequireSimpleMDE"] = true
...
func uploadAttachment(c *context.RestContext, allowedTypes []string) {
	file, header, err := c.Req.FormFile("file")
	if err != nil {
		//c.Error(500, fmt.Sprintf("FormFile: %v", err)) //TODO
		return
	}
	defer file.Close()
	...
	if !allowed {
		//c.Error(400, ErrFileTypeForbidden.Error()) //TODO
		return
	}

	attach, err := models.NewAttachment(header.Filename, buf, file)
	if err != nil {
		//c.Error(500, fmt.Sprintf("NewAttachment: %v", err)) //TODO
		return
	}
...
func UploadIssueAttachment(c *context.RestContext) {
	if !setting.AttachmentEnabled {
		c.NotFound()
		return
	}
...
func viewIssue(c *context.RestContext, isPullList bool) {
	c.Data["RequireHighlightJS"] = true
	c.Data["RequireDropzone"] = true
	c.Data["RequireDatetimepicker"] = true
	renderAttachmentSettings(c)
...
func ViewIssue(c *context.RestContext) {
	viewIssue(c, false)
}

func ViewPull(c *context.RestContext) {
	viewIssue(c, true)
}

func getActionIssue(c *context.RestContext) *models.Issue {
...
func UpdateIssueTitle(c *context.RestContext) {
...
func UpdateIssueContent(c *context.RestContext) {
	issue := getActionIssue(c)
	if c.Written() {
		return
	}
...
func UpdateIssueLabel(c *context.RestContext) {
	issue := getActionIssue(c)
	if c.Written() {
		return
	}
...
func UpdateIssueMilestone(c *context.RestContext) {
	issue := getActionIssue(c)
	if c.Written() {
		return
	}
...
func UpdateIssueAssignee(c *context.RestContext) {
	issue := getActionIssue(c)
	if c.Written() {
		return
	}
...
func NewComment(c *context.RestContext, f form.CreateComment) {
	issue := getActionIssue(c)
	if c.Written() {
		return
	}
...
func UpdateCommentContent(c *context.RestContext) {
...
func DeleteComment(c *context.RestContext) {
	comment, err := models.GetCommentByID(c.ParamsInt64(":id"))
	if err != nil {
		c.NotFoundOrServerError("GetCommentByID", models.IsErrCommentNotExist, err)
		return
...
func Labels(c *context.RestContext) {
...
func InitializeLabels(c *context.RestContext, f form.InitializeLabels) {
	if c.HasError() {
...
func NewLabel(c *context.RestContext, f form.CreateLabel) {
	c.Data["Title"] = c.Tr("repo.labels")
	c.Data["PageIsLabels"] = true
...
func UpdateLabel(c *context.RestContext, f form.CreateLabel) {
...
func DeleteLabel(c *context.RestContext) {
...
func Milestones(c *context.RestContext) {
...
func NewMilestone(c *context.RestContext) {
...
func NewMilestonePost(c *context.RestContext, f form.CreateMilestone) {
	c.Data["Title"] = c.Tr("repo.milestones.new")
	c.Data["PageIsIssueList"] = true
	c.Data["PageIsMilestones"] = true
	c.Data["RequireDatetimepicker"] = true
...
func EditMilestone(c *context.RestContext) {
...
func EditMilestonePost(c *context.RestContext, f form.CreateMilestone) {
	c.Data["Title"] = c.Tr("repo.milestones.edit")
	c.Data["PageIsMilestones"] = true
	c.Data["PageIsEditMilestone"] = true
	c.Data["RequireDatetimepicker"] = true
...
func ChangeMilestonStatus(c *context.RestContext) {
	m, err := models.GetMilestoneByRepoID(c.Repo.Repository.ID, c.ParamsInt64(":id"))
	if err != nil {
		if models.IsErrMilestoneNotExist(err) {
			c.Handle(404, "", err)
...
func DeleteMilestone(c *context.RestContext) {
