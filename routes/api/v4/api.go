...
import (
	...
	"github.com/gogits/gogs/routes/api/v4/admin"
	"github.com/gogits/gogs/routes/api/v4/repo"
	"github.com/gogits/gogs/routes/api/v4/org"
	"github.com/gogits/gogs/routes/api/v4/user"
	"github.com/gogits/gogs/routes/api/v4/misc"
)
...
func RegisterRoutes(m *macaron.Macaron) {
	...
	reqSignIn := context.RestToggle(&context.ToggleOptions{SignInRequired: true})
	ignSignIn := context.RestToggle(&context.ToggleOptions{SignInRequired: setting.Service.RequireSignInView})
	ignSignInAndCsrf := context.RestToggle(&context.ToggleOptions{DisableCSRF: true})
	reqSignOut := context.RestToggle(&context.ToggleOptions{SignOutRequired: true})
	...
	m.Group("/v4", func() {
		...
		//Account
		m.Get("/account", user.GetAccount)
		m.Get("/profile-info", misc.GetProfileInfo)

		m.Group("/explore", func() {
			m.Get("", func(c *context.RestContext) {
		...
		}, reqSignIn, func(c *context.RestContext) {
		...
		m.Group("", func() {
			...
			m.Get("/attachments/:uuid", func(c *context.RestContext) {
		...
		m.Group("/:username/:reponame", func() {
			...
			}, repo.MustBeNotBare, reqRepoWriter, func(c *context.RestContext) {
			...
			}, repo.MustBeNotBare, reqRepoWriter, context.RepoRef(), func(c *context.RestContext) {
		...
		m.Group("/:username/:reponame", func() {
			...
			}, repo.MustBeNotBare, func(c *context.RestContext) {
				c.Data["PageIsViewFiles"] = true
			})
