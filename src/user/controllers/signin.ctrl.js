    ...
    angular.module('gitlabKBApp.user').controller('SigninController', [
        ...
        function($scope, $http, $state, AuthService, store, host_url, enable_signup) {
            ...
            $scope.oauth = function() {
                var authWindow = window.open('/api/oauth?provider='+git_service_vendor, 'Auth', "menubar=no,location=0,resizable=yes,scrollbars=yes,status=0");
                var listener = function(event) {
                    authWindow.close();
                    window.removeEventListener('message', listener);
                    $http.post('/api/oauth', {
                        code: event.data,
                        provider: git_service_vendor
