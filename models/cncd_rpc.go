...
func (s *RPC) Update(c context.Context, id string, state rpc.State) error {
	...
	repo, err := GetRepositoryByID(build.RepoID)
	if err != nil {
	...
	if state.Exited {
		...
		proc.State = StatusSuccess
		if state.ExitCode != 0 || state.Error != "" {
			proc.State = StatusFailure
		}
	} else {
		proc.Started = state.Started
		proc.State = StatusRunning
	}

	if err := ProcUpdate(proc); err != nil {
		log.Printf("error: rpc.update: cannot update proc: %s", err)
	}

	build.Procs, _ = ProcList(build)
	build.Procs = Tree(build.Procs)
...
func (s *RPC) Upload(c context.Context, id string, file *rpc.File) error {
	...
	proc, err := ProcChild(build, pproc.PID, file.Proc)
	if err != nil {
...
func (s *RPC) Init(c context.Context, id string, state rpc.State) error {
	...
	build, err := GetBuild(proc.BuildID)
	if err != nil {
...
func (s *RPC) Done(c context.Context, id string, state rpc.State) error {
	...
	procs, _ := ProcList(build)
	for _, p := range procs {
		if p.Running() && p.PPID == proc.PID {
			p.State = StatusSkipped
			if p.Started != 0 {
				p.State = StatusSuccess // for deamons that are killed
				p.Stopped = proc.Stopped
			}
			if err := ProcUpdate(p); err != nil {
	...
	if !running {
		...
		// update the status
		//user, err := s.store.GetUser(repo.ID)
		//if err == nil {
...
func (s *RPC) checkCancelled(pipeline *rpc.Pipeline) (bool, error) {
	...
	proc, err := ProcLoad(pid)
	if err != nil {
		return false, err
	}
	if proc.State == StatusKilled {
		return true, nil
	}
	return false, err
}
